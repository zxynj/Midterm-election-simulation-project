...
df1a = df1a_raw %>% group_by(sampleSize,method,trueValue) %>% mutate(ID = 1:n()) %>% ungroup() %>% mutate(method=as.factor(method))
...
df1b = df1b_raw %>% group_by(sampleSize,method,trueValue) %>% mutate(ID = 1:n()) %>% ungroup() %>% mutate(method=as.factor(method))
...
df2 = df2raw %>% group_by(mixRatio,sampleSize,method,trueValue) %>% mutate(ID = 1:n()) %>% ungroup() %>% mutate(method=as.factor(method))

...
This is an R Markdown document for project 1: <http://utdallas.edu/~ammann/stat6341/node10.html>.
...
Generate 2 simulated data, for example:

```{r data, echo=T}
set.seed(321)
df = rmultinom(2, size = sampleSizeVector[1], prob = p)
df
```

Each column is one poll. You can see the distribtuion is about `r p`. Then a function is designed to compute the confidence interval using method 1 and method 2. **This function is going to be the importance one, because it is the most basic part for confidence interval computation regardless what's the distribution or whether it is mixed or not.**
...
confiFun <- function(singleSample,type,i,j){
  N = sum(singleSample)   #the column sum; sample size
...
Since we have two column, the result is the length 2 list, where each element is a dataframe. Looks good. Then we have to try different sample size, which can be done using a function. And we can embed all 3 pair difference to one sampleSize. Check the **apply** build-in function. Check the **Reduce** build-in function.
...
Then we have all three pair difference in one shot. The sampleSize column is to distinguish the result from different sampleSize. Let's try two sample size: 500, 1000,1500. Check the **lapply** build-in function.
...
Therefore, we get the result for one specific distribtuion. Combine them into one dataframe and save it for plotting. 
...
Once we have the similiar data at question 1. We can just simply apply the defined functions.
...
Then we can design a function that generate mix data and calucate the pair difference, which is similar to confiDataFun. 
